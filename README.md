...
> 实现状态说明（2026-03-13）：
> README 同时包含“当前可用能力”和“项目设计目标”的汇总说明，并非所有条目都已达到完整产品化或验收完成状态。
> 当前仓库已经具备基础聊天闭环、审批恢复、文件/命令工具、Web 搜索抓取、浏览器自动化基础能力、SQLite 记忆系统。
> 仍未完全闭环或仅部分实现的能力包括：Daemon 重启后的 checkpoint 自动恢复、完整 MCP/扩展授权调用链、文档中定义的完整 E2E/CI 门禁。
...
> 注：`~/.aha/extensions/` 目录和扩展安装/运行基础代码已存在，但完整 MCP 扩展产品链路仍在完善中。

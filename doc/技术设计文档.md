...
> 当前未完全实现或未闭环的能力包括：扩展/MCP 的完整安装授权调用链、文档中描述的完整队列/锁/回滚体系、严格意义上的“协议全链路验收”、非审批中的运行中任务在 Daemon 重启后的自动续跑。
...
> 当前状态：部分实现。数据库、记忆、网关启动与前端取 token 已落地；审批等待中的任务已支持重启恢复与重连回放，但扩展扫描、自动启动已启用扩展、运行中任务自动续跑等流程尚未完全打通。
